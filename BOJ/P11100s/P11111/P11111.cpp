...
const int SIZE = 3333;
ll n, m, start, fin, MCMF, dx[4] = {1, -1, 0, 0}, dy[4] = {0, 0, 1, -1};
vector<Edge*> adj[SIZE];
string tofu[SIZE];

int get_cost(char x, char y)
{
    if(x == 'A' && y == 'A') return 10;
    if((x == 'A' && y == 'B') || (x == 'B' && y == 'A')) return 8;
    if((x == 'A' && y == 'C') || (x == 'C' && y == 'A')) return 7;
    if((x == 'A' && y == 'D') || (x == 'D' && y == 'A')) return 5;
    if(x == 'B' && y == 'B') return 6;
    if((x == 'B' && y == 'C') || (x == 'C' && y == 'B')) return 4;
    if((x == 'B' && y == 'D') || (x == 'D' && y == 'B')) return 3;
    if(x == 'C' && y == 'C') return 3;
    if((x == 'C' && y == 'D') || (x == 'D' && y == 'C')) return 2;
    if(x == 'D' && y == 'D') return 2;
    if(x == 'F' && y == 'F') return 0;
    return 1;
}
...
int main()
{
        ...
        for(int j = 0; j < m; j++)
        {
            if((i+j)&1)
            {
                for(int z = 0; z < 4; z++)
                {
                    ...
                    Edge *e1 = new Edge(nx*m+ny, 1, -cost), *e2 = new Edge(i*m+j, 0, cost);
                    e1->dual = e2;
                    e2->dual = e1;

                    adj[i*m+j].push_back(e1);
                    adj[nx*m+ny].push_back(e2);
                }
            }
        }
    }
    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < m; j++)
        {
            if((i+j)&1)
            {
                Edge *e1 = new Edge(i*m+j, 1, 0), *e2 = new Edge(start, 0, 0);
                e1->dual = e2;
                e2->dual = e1;
    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < m; j++)
        {
            Edge *e1 = new Edge(fin, 1, 0), *e2 = new Edge(i*m+j, 0, 0);
            e1->dual = e2;
            e2->dual = e1;
