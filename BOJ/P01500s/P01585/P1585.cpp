...
const int SIZE = 1111;
ll n, a[50], b[50], t, f, start, fin, MCMF, cnt;
...
int main()
{
    ...
    for(int z = 1; z > -2; z -= 2)
    {
        ...
        start = 2*2*n+1, fin = 2*2*n+2;
        for(int i = 0; i < 2*n; i++)
        {
            Edge *e1 = new Edge(i*2+1, 1, 0), *e2 = new Edge(i*2, 0, 0);
            e1->dual = e2;
            e2->dual = e1;
        ...
        for(int i = 0; i < n; i++)
        {
            for(int j = 0; j < n; j++)
            {
                ...
                {
                    int s = b[j] - a[i];
                    Edge *e1 = new Edge((j+n)*2, 1, t > s ? z*min(f, (t-s)*(t-s)) : 0), *e2 = new Edge(i*2+1, 0, t > s ? -z*min(f, (t-s)*(t-s)) : 0);
                    e1->dual = e2;
                    e2->dual = e1;

                    adj[i*2+1].push_back(e1);
                    adj[(j+n)*2].push_back(e2);
                }
            }
        }
        for(int i = 0; i < n; i++)
        {
            Edge *e1= new Edge(i*2, 1, 0), *e2 = new Edge(start, 0, 0);
            e1->dual = e2;
            e2->dual = e1;
        ...
        for(int i = n; i < 2*n; i++)
        {
            Edge *e1= new Edge(fin, 1, 0), *e2 = new Edge(i*2+1, 0, 0);
            e1->dual = e2;
            e2->dual = e1;
